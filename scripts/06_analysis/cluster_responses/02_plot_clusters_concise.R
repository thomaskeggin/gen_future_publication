...
aspect_ratio <-
  max(heat_scaled$`Dispersal range (km / year)`) / max(heat_scaled$`Adaptive rate (°C / year)`)
...
plot_kmeans_polygons <-
  ...
  # polygons
  geom_polygon(aes(x = `Dispersal range (km / year)`,
                   y = `Adaptive rate (°C / year)`,
                   fill = factor(kmeans_cluster_name)),
               alpha = 0.7) +
  
  # dispersal thresholds
  geom_vline(xintercept = dispersal_thresholds[1:9],
             linetype = "dashed",
             alpha = 0.45) +
