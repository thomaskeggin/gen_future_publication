...
# plot gggraph# plot gradient function
...
flow_colours <-
  c("Causes maladaptation" = "#4D262D",
    "Decreases fitness" = "#CC6677",
    "No gene flow" = "white",
    "Increases fitness" = "#88CCEE")
...
plot_dispersal <-
  ...
  # end
  geom_point(aes(x = sst_dispersal,
                 y = trait_dispersal),
             fill = "grey",
             size = point_size,
             shape = 22) +
...
plot_gene_flow <-
  ...
  lims(x = limits,
       y = limits) +
  labs(title = "Adaptation and dispersal with gene flow",
       y = "",
       x = "Environmental temperature",
       fill = "Fitness consequence\nof gene flow") +
  coord_fixed() +
